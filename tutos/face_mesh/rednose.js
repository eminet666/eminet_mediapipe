  ...
  if (video.currentTime !== lastVideoTime) {
    ...
    if (results.faceLandmarks) {
      for (const landmarks of results.faceLandmarks) {
        ...
        // C. RED NOSE
        // Récupérer le point du bout du nez (Index 1)
        ...
        // Dessiner un rond rouge sur le nez
        ctx.beginPath();
        ctx.arc(x, y, 25, 0, 2 * Math.PI); // Rayon de 25px
        ctx.fillStyle = "red";
        ctx.fill();

        // Ajouter un petit reflet blanc pour faire "plastique"
        ctx.beginPath();
